...
func (c *RPCClient) sendRequest(ctx context.Context, addr string, req *tikvrpc.Request, timeout time.Duration) (resp *tikvrpc.Response, err error) {
	...
	if config.GetGlobalConfig().TiKVClient.MaxBatchSize > 0 && enableBatch && req.Type != tikvrpc.CmdPrewrite && req.Type != tikvrpc.CmdCommit && req.Type != tikvrpc.CmdPessimisticLock {
